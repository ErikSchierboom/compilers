...
class Lexer(string source)
{
    public List<TokenKind> Lex()
    {
        ...
        while (position < source.Length)
        {
            ...
            switch (source[position])
            {
                ...
                case '=':
                    position++;
                    tokens.Add(TokenKind.Equal);
                    break;
                case '+':
                    position++;
                    tokens.Add(TokenKind.Plus);
                    break;
                case ';':
                    position++;
                    tokens.Add(TokenKind.Semicolon);
                    break;
                case >= '0' and <= '9':
                    while (position < source.Length && char.IsDigit(source[position]))
                        position++;
                    
                    tokens.Add(TokenKind.Number);
                    break;
                case >= 'a' and <= 'z' or >= 'A' and <= 'Z':
                    while (position < source.Length && char.IsLetterOrDigit(source[position]))
                        position++;
                    
                    var identifier = source[start..position];
                    if (identifier == "var")
                        tokens.Add(TokenKind.Var);
                    else
                        tokens.Add(TokenKind.Identifier);
        ...
        tokens.Add(TokenKind.EndOfFile);
...
public sealed record NumberExpression : Expression;

class Parser(List<TokenKind> tokens)
{
    ...
    private bool IsEndOfFile() => Token == TokenKind.EndOfFile;

    private TokenKind Token => tokens[_position];
}
